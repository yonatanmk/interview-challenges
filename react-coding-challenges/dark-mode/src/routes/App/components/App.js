...
function App() {
  const [mode, setMode] = useState(MODE_ENUM.LIGHT);
  const toggleMode = () => {
    setMode(mode === MODE_ENUM.LIGHT ? MODE_ENUM.DARK : MODE_ENUM.LIGHT);
  };
  return (
    <div
      className={classnames({
        app: true,
        ["dark-mode"]: mode === MODE_ENUM.DARK,
      })}
    >
      <div className="level">
        <div>
        ...
        <button
          ...
          onClick={toggleMode}
        >
          <FontAwesomeIcon icon={mode === MODE_ENUM.LIGHT ? faMoon : faSun} />
        </button>
      </div>
